...
class EmailHeader:
    ...
    def pass_filter(self, sender, sbj_1, sbj_2):
        booking_type = "NA"
        if sender == email.utils.parseaddr(str(self.From))[1]:
            if str(self.Subject).lower().startswith(sbj_1):
                booking_type = "chg"
                return booking_type
            elif str(self.Subject).lower().startswith(sbj_2):
                booking_type = "new"
                return booking_type
            else:
                return  booking_type
...
class EmailInbox:
    ...
    def fetch_mail(self, sender, sbj_chg, sbj_new, filter="UnSeen"):
        """
    ...
    if my_email.connect("EMAIL-ADDRESS", "EMAIL-PASSWORD"):
        ...
        if not inbox is None:
            mail = inbox.fetch_mail(
                'senders-email', 'starts-with-1', 'starts-with-2', filter="All"
                )
            for m in mail:
                print(m)
                m.save_attachment("c:/tmp")
        my_email.disconnect()
    else:
